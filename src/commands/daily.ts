...
export function registerDailyCommands(bot: any): void {
  bot.command('vocab', async (ctx: Context) => {
    ...
    await ctx.reply(lang === 'vi' ? '⏳ Đang tìm kiếm từ vựng IELTS hay cho bạn...' : '⏳ Finding a great IELTS vocabulary for you...');
  ...
  bot.command('grammar', async (ctx: Context) => {
    ...
    await ctx.reply(lang === 'vi' ? '⏳ Đang soạn cấu trúc ngữ pháp...' : '⏳ Preparing a grammar structure...');
    ...
    try {
      ...
      const msg = lang === 'vi'
        ? \`📝 **NGỮ PHÁP NÂNG CAO**\n━━━━━━━━━━━━━━━━━━━━━━\n\n📌 **\${data.name}**\n📐 **Cấu trúc:** \`\${data.formula}\`\n\n💡 **Cách dùng (IELTS):** \${data.usage_vi}\n\n📝 **Ví dụ:**\n- \${data.example_en}\n- *(\${data.example_vi})*\n\n*(Cấu trúc này đã được lưu lại để ôn tập)*\`
        : \`📝 **ADVANCED GRAMMAR**\n━━━━━━━━━━━━━━━━━━━━━━\n\n📌 **\${data.name}**\n📐 **Formula:** \`\${data.formula}\`\n\n💡 **IELTS Usage:** \${data.usage_en}\n\n📝 **Example:**\n- \${data.example_en}\n\n*(This structure has been saved for review)*\`;

      await ctx.reply(msg, { parse_mode: 'Markdown' });
    } catch (e) {
      console.error(e);
      await ctx.reply(lang === 'vi' ? '❌ Lỗi khi lấy ngữ pháp. Thử lại nhé.' : '❌ Error fetching grammar. Try again.');
    }
  });
  ...
  bot.command('phrase', async (ctx: Context) => {
    ...
    await ctx.reply(lang === 'vi' ? '⏳ Đang lấy cụm từ/thành ngữ...' : '⏳ Fetching phrase/idiom...');
    ...
    try {
      ...
      const msg = lang === 'vi'
        ? \`💬 **CỤM TỪ / THÀNH NGỮ**\n━━━━━━━━━━━━━━━━━━━━━━\n\n🔥 **\${data.phrase}**\n\n📖 **Ý nghĩa:** \${data.meaning_vi}\n🎯 **Áp dụng:** \${data.context}\n\n📝 **Ví dụ:**\n- \${data.example_en}\n- *(\${data.example_vi})*\n\n*(Cụm từ này đã được lưu lại để ôn tập)*\`
        : \`💬 **PHRASE / IDIOM**\n━━━━━━━━━━━━━━━━━━━━━━\n\n🔥 **\${data.phrase}**\n\n📖 **Meaning:** \${data.meaning_en}\n🎯 **Context:** \${data.context}\n\n📝 **Example:**\n- \${data.example_en}\n\n*(This phrase has been saved for review)*\`;

      await ctx.reply(msg, { parse_mode: 'Markdown' });
    } catch (e) {
      console.error(e);
      await ctx.reply(lang === 'vi' ? '❌ Lỗi khi lấy cụm từ. Thử lại nhé.' : '❌ Error fetching phrase. Try again.');
    }
  });
